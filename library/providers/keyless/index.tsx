...
const AptosKeylessContext = createContext<AptosKeylessContextType | undefined>(
  undefined
);
...
export const AptosKeylessProvider: React.FC<{
  ...
  const [config, setConfig] = useState<AptosKeylessConfig>({
    ...defaultConfig,
    ...initialConfig,
  });
  const [aptos, setAptos] = useState(
    () => new Aptos(new AptosConfig({ network: config.network }))
  );
  ...
  const deriveAccount = useCallback(
    async (jwt: string, ekp: EphemeralKeyPair) => {
  ...
  useEffect(() => {
    const loadAccount = async () => {
      ...
      if (storedJwt && storedEkp) {
        try {
          const ekp = EphemeralKeyPair.fromBytes(
            new Uint8Array(JSON.parse(storedEkp))
          );
  ...
  const handleCallback = useCallback(
    async (url: string) => {
      ...
      const storedEkp = localStorage.getItem("@aptos/ephemeral_key_pair");
      const ekp = storedEkp
        ? EphemeralKeyPair.fromBytes(new Uint8Array(JSON.parse(storedEkp)))
        : null;
...
export const useAptosKeyless = () => {
  ...
  if (context === undefined) {
    throw new Error(
      "useAptosKeyless must be used within an AptosKeylessProvider"
    );
  }
  return context;
};
